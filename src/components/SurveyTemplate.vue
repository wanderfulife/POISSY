...
const filterStreets = () => {
  ...
  const streetFile = currentQuestion.value?.streetFile;
  const list = streetFile && props.streetsMap[streetFile]
    ? props.streetsMap[streetFile]
    : props.streetsList;
  filteredStreets.value = Array.isArray(list)
    ? list.filter(street => street.toLowerCase().includes(inputLower))
    : [];
};
